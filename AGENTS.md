# Agent Instructions

This is an open-source, agent-agnostic skill for managing developer task backlogs via markdown files. It targets individual developers using AI coding agents (Claude Code, Cursor, Windsurf, etc.).

The `.backlog/` in this repo is not an example — it's the real backlog for this project, managed by the skill it defines. Dogfooding by design.

Always use the agent-backlog skill when making changes to this project.
